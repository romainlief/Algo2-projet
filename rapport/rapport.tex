...
\begin{table}[h]
  ...
  findStop & Stop & Recherche un arrêt dans la map des arrêts à partir de son nom. \\
  \hline
  \end{tabular}
...
\begin{algorithm}[H]
  ...
  Trouver \texttt{starting\_stop} et \texttt{end\_stop} dans \texttt{stopMap} \;
...
Ce tri a une complexité en $O(n \log n)$, où $n$ est le nombre de connexions. Nous utilisons Collections.sort() qui utilise un 
