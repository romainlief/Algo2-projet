...
La complexité de l'algorithme en lui est donc de $\Theta(n)$, où $n$ est le nombre de connexions.
...
Comme dit dans la subsection précédente, l'algorithme de base est en $\Theta(n)$, où $n$ est le nombre de connexions.
Cependant, nous devons aussi prendre en compte les marches à pied dedans et les optimisations que nous avons faites.
Lorsque nous lancons notre algorithme, nous allons checker si nous avons des marches à pied à chaque arrêt.
Pour ceci, nous allons devoir parcourir l'ensemble des \emph{Walk} de chaque arrêt.
Nous avons donc une complexité de $\Theta(n * m)$, où $n$ est le nombre d'arrêts et $m$ le nombre de marches à pied par arrêt.
Avec ça, nous allons maintenant parler des deux optimisations que nous avons faites:
\begin{itemize}
    \item \textbf{Starting criterion:} Avec l'algorithme élémentaire qu'est la recherche dichotomique (en $O(\log n)$), nous avons pu trouver le premier arrêt 
    qui est supérieur ou égal à l'heure de départ. Ceci nous permet de ne pas parcourir toutes les connexions, mais de commencer à partir de l'indice trouvé.
    Cela nous permet de réduire le nombre d'itérations nécessaires pour trouver le chemin optimal. Grâce à cela, L'algorithme passe de $\Theta(n)$ à $\Theta(n - k)$ avec k le 
    nombre de connexions avant l'heure de départ. Cela nous permet de grandement réduire le temps d'exécution de l'algorithme, et encore plus si l'heure de départ est
    est élevée, vu que la liste est en moins en mois grande.
    Si l'heure entrée par l'utilisateur est la 1ere dans la liste de connexion, alors nous devons quand même tout parcourir et sommes donc en $\Theta(n)$.
    \item \textbf{Stopping criterion:} Cette optimisation, permet qu'une fois que nous avons atteint la destination ou que toutes 
    les connexions possibles ont été explorées, 
    nous arrêtons l'algorithme. Cela permet d'éviter de parcourir inutilement des connexions qui ne contribuent pas à améliorer le chemin trouvé. 
    En pratique, cela signifie que si l'heure d'arrivée à un arrêt est déjà optimale (c'est-à-dire qu'aucune connexion ultérieure ne peut améliorer cette heure), 
    nous pouvons ignorer les connexions restantes. Cette optimisation réduit le nombre d'itérations nécessaires, surtout lorsque la destination est atteinte 
    rapidement dans la liste triée des connexions. En terme de complexité, si l'on part de l'algorithm en $\Theta(n)$, alors on passe en $\Theta(n - j)$ 
    ou j est le nombre de connexion qu'on a pu enlever. Ceci nous permet d'avoir de meilleures performences globale.
\end{itemize}

Au final si l'on assemble tout ça, nous avons une complexité de $\Theta((n - k - j) * m)$ pour le coeur de l'algorithm.
La reconstruction du chemin se fait en parcourant les connexions de path et donc nous sommes en $\Theta(p)$ ou p est le nombre de connexions dans path.

Il existe une troisième optimisation du CSA s'appellant "Limited Walking", cependant celle-ci est plus compliquée a mettre en place et par 
manque de temps, nous n'avons pas eu le temps de l'implémenter.
...
Et bien que nous ne puissions pas le voir sur le graphe, il est tout de même présent et permet de réduire le temps d'exécution de l'algorithme.
